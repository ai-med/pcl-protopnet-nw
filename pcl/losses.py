...
class GeneralizedSupervisedNTXenLoss(nn.Module):
    ...
    def forward(self, z_i, z_j, labels):
        ...
        all_labels = labels.view(N, -1).repeat(2, 1).detach().cpu().numpy() # [2N, *] -> Why 2N? Because we need to account for the other N samples which are augmented versions of the ori N samples
