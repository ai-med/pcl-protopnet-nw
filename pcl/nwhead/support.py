...
class SupportSetEval(SupportSet):
    ...
    def _build_full_loader(self):
        ...
        #         env, batch_size=128, shuffle=False, num_workers=0) for env in self.full_datasets] # This batch size is too big for our 3D scans
        return [torch.utils.data.DataLoader(
                env, batch_size=8, shuffle=False, num_workers=0) for env in self.full_datasets]